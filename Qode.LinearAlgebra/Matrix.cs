...
namespace Qode.LinearAlgebra
{
        ...
        where T : INumberBase<T>
    {
        ...
        public Matrix(T[] vectorArray)
        {
            var matrixArray = new T[1, vectorArray.Length];
            for (int i = 0; i < vectorArray.Length; i++)
            {
                matrixArray[0, i] = vectorArray[i];
            }

            InitializeFromArray(matrixArray);
        }
        ...
        static public Matrix<T> Vector(int elements, bool isColumnVector = true)
        {
            if (isColumnVector)
            {
                return new Matrix<T>(1, elements);
            }
            else
            {
                return new Matrix<T>(elements, 1);
            }
        }
        ...
        public Matrix<T> Add(Matrix<T> matrix)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                this[row, column] = this[row, column] + matrix[row, column];
            }
        ...
        public Matrix<T> Multiply(Matrix<T> matrix)
        {
            ...
            for (int row = 0; row < Rows; row++)
            {
                for (int column = 0; column < matrix.Columns; column++)
                {
                    ...
                    {
                        product[row, column] += this[row, element] * matrix[element, column];
                    }
                }
        ...
        /// </summary>
        /// <param name="matrix">A matrix to compute a tensor product with.</param>
        /// <returns>The tensor product matrix.</returns>
        public Matrix<T> TensorProduct(Matrix<T> matrix)
        {
            var product = new Matrix<T>(matrix.Rows * Rows, matrix.Columns * Columns);
            for (int i = 0; i < product.Rows; i++)
            {
                for (int j = 0; j < product.Columns; j++)
                {
                    var a = this[i / matrix.Rows, j / matrix.Columns];
                    var b = matrix[i % matrix.Rows, j % matrix.Columns];
                    product[i, j] = a * b;
                }
            }
        ...
        public double Determinant
        {
            get
            {
                ...
                if (Order == 1)
                    return double.CreateSaturating(this[0, 0]);

                if (Order == 2)
                {
                    // "\ minus /"
                    var secondOrderDeterminant = this[0, 0] * this[1, 1] - this[0, 1] * this[1, 0];
                    return double.CreateSaturating(secondOrderDeterminant);
                }
                ...
                {
                    determinant += double.CreateSaturating(this[0, position]) * AlgebraicComplement(0, position);
                }
